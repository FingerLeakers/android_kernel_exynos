...
#define DIT_DEBUG_PSEUDO_PCTPROC
#define DIT_DEBUG_FORWARD_IPEF_SEQUENCER
#endif
...
#define DIT_PKTPROC_MAX_QUEUE	4 /* PKTPROC_MAX_QUEUE */
...
struct dit_pacproc_t {
	unsigned int num_queue;
	struct dit_pktproc_queue_t {
		unsigned int size;
		unsigned int wp;
		unsigned int rp;
	} q[DIT_PKTPROC_MAX_QUEUE];
};
...
struct dit_stat_perf_t {
	...
	struct dit_pacproc_t cp_pktproc;
};
...
struct dit_napi_t {
	...
	struct napi_struct backlog_desc;
...
struct dit_dev_info_t {
	...
	struct hrtimer sched_desc_timer;
	struct hrtimer sched_fwd_timer;

	struct dit_pacproc_t pktproc;
