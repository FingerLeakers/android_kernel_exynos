...
int dsp_pm_enable(struct dsp_pm *pm)
{
	...
	if (!pm->devfreq[DSP_DEVFREQ_DSP].current_qos) {
		pm_qos_add_request(&pm->mif, PM_QOS_BUS_THROUGHPUT, 1352000);
		dsp_info("devfreq[mif] set high\n");
	} else {
		pm_qos_add_request(&pm->mif, PM_QOS_BUS_THROUGHPUT, 0);
