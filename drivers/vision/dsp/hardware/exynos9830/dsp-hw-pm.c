...
int dsp_pm_probe(struct dsp_system *sys)
{
	...
	snprintf(devfreq[DSP_DEVFREQ_MIF].name, DSP_DEVFREQ_NAME_LEN, "mif");
	devfreq[DSP_DEVFREQ_MIF].count = sizeof(mif_table) / sizeof(*mif_table);
	devfreq[DSP_DEVFREQ_MIF].table = mif_table;
	devfreq[DSP_DEVFREQ_MIF].class_id = PM_QOS_BUS_THROUGHPUT,
	devfreq[DSP_DEVFREQ_MIF].default_qos = 6;
	devfreq[DSP_DEVFREQ_MIF].resume_qos = -1;
	devfreq[DSP_DEVFREQ_MIF].current_qos = -1;
